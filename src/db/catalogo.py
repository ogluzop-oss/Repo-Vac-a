...
def _tienda(id_tienda="auto"):
    """Id de tienda para columnas INT del catálogo (`catalogo_*.id_tienda`). Coacciona el contexto
    (código 'ALMC'→0) con el helper canónico, evitando el error 1366; None si no hay tienda."""
    try:
        from src.db.empresa import tienda_actual_id, tienda_actual_id_int
        v = id_tienda if id_tienda != "auto" else tienda_actual_id()
