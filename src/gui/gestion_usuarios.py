...
_DENOM = [
    ("1 ct",   0.01, "M"), ("2 ct",   0.02, "M"),
    ("5 ct",   0.05, "M"), ("10 ct",  0.10, "M"),
    ("20 ct",  0.20, "M"), ("50 ct",  0.50, "M"),
    ("1 €",    1.00, "M"), ("2 €",    2.00, "M"),
    ("5 €",    5.00, "B"), ("10 €",  10.00, "B"),
    ("20 €",  20.00, "B"), ("50 €",  50.00, "B"),
    ("100 €", 100.00, "B"), ("200 €", 200.00, "B"),
    ("500 €", 500.00, "B"),
]
...
_DENOM_ICON_CACHE: dict = {}
_DENOM_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "assets", "denominaciones")


def _make_denom_icon(label: str, typ: str) -> "QIcon":
    """Loads PNG from assets/denominaciones/ if present, otherwise renders programmatically."""
    if label in _DENOM_ICON_CACHE:
        return _DENOM_ICON_CACHE[label]

    safe = label.replace(" ", "").replace("€", "e")
    png_path = os.path.join(_DENOM_DIR, f"{safe}.png")
    if os.path.isfile(png_path):
        raw = QPixmap(png_path)
        if not raw.isNull():
            sz = 56 if typ == "M" else 66
            # Encuadrar en lienzo cuadrado para que todas las monedas queden al mismo tamaño
            ...
            _p = _QP(square)
            _p.drawPixmap((side - raw.width()) // 2, (side - raw.height()) // 2, raw)
            _p.end()
            square = square.scaled(sz, sz, Qt.AspectRatioMode.IgnoreAspectRatio,
                                   Qt.TransformationMode.SmoothTransformation)
            icon = QIcon(square)
            _DENOM_ICON_CACHE[label] = icon
            return icon
    ...
    cx_c, cy_c = sz // 2, sz // 2  # canvas center

    def _eu_stars(painter, cx, cy, r, n=12, star_r=1.8, color=QColor("#FFD700")):
        """Draw N EU stars in a circle of radius r, centered at (cx, cy)."""
        ...
        for i in range(n):
            ...
            painter.drawEllipse(QRectF(sx - star_r, sy - star_r, star_r * 2, star_r * 2))

    if typ == "M":
        val_str = label.replace(" ct", "").replace(" €", "").replace(",", ".")
        val = float(val_str) if val_str.replace(".", "").isdigit() else 0.0
        if "ct" in label:
            val /= 100.0  # convert ct → €

        if val <= 0.05:         # 1ct, 2ct, 5ct — copper
            c_edge, c_rim, c_face = "#4A1A08", "#8B3218", "#C1522D"
            bimetal = False; c_in_rim = c_in_face = None
        elif val <= 0.50:       # 10ct, 20ct, 50ct — Nordic gold
            c_edge, c_rim, c_face = "#5A3D00", "#9A6A00", "#D4A017"
            bimetal = False; c_in_rim = c_in_face = None
        elif val == 1.0:        # 1€ — gold ring, silver center
            c_edge, c_rim, c_face = "#5A3D00", "#9A6A00", "#C9A010"
            bimetal = True; c_in_rim, c_in_face = "#3A4A58", "#7A9AB8"
        else:                   # 2€ — silver ring, gold center
            c_edge, c_rim, c_face = "#3A4A58", "#5A7090", "#8AAAC8"
            bimetal = True; c_in_rim, c_in_face = "#5A3D00", "#C9A010"

        R = sz // 2 - 1
        # Outer edge (dark)
        p.setBrush(QColor(c_edge)); p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QRectF(cx_c - R, cy_c - R, R * 2, R * 2))
        # Main rim
        Rr = R - 3
        p.setBrush(QColor(c_rim)); p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QRectF(cx_c - Rr, cy_c - Rr, Rr * 2, Rr * 2))
        # Face
        Rf = R - 6
        p.setBrush(QColor(c_face)); p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QRectF(cx_c - Rf, cy_c - Rf, Rf * 2, Rf * 2))

        if bimetal:
            Ri = R * 10 // 22  # inner circle radius
            p.setBrush(QColor(c_in_rim)); p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(QRectF(cx_c - Ri, cy_c - Ri, Ri * 2, Ri * 2))
            Rif = Ri - 3
            p.setBrush(QColor(c_in_face)); p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(QRectF(cx_c - Rif, cy_c - Rif, Rif * 2, Rif * 2))
            # Shine on inner circle
            p.setBrush(QColor(255, 255, 255, 40)); p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(QRectF(cx_c - Rif, cy_c - Rif, Rif * 1.0, Rif * 0.7))
            # EU stars in inner ring gap (between Rif+3 and Ri)
            _eu_stars(p, cx_c, cy_c, Ri - 1, n=8, star_r=1.3)
        else:
            # EU stars ring on outer face
            _eu_stars(p, cx_c, cy_c, Rf - 5, n=12, star_r=1.5)

        # Upper-left shine on face
        p.setBrush(QColor(255, 255, 255, 55)); p.setPen(Qt.PenStyle.NoPen)
        shine_r = Rf * 0.55
        p.drawEllipse(QRectF(cx_c - Rf + 2, cy_c - Rf + 2, shine_r, shine_r * 0.65))

        # Denomination text
        txt_col = QColor("#2A1000") if val <= 0.50 else QColor("#0D1020")
        p.setPen(txt_col)
        num_only = label.replace(" ct", "ct").replace(" €", "€")
        fz = 11 if len(num_only) <= 3 else 9
        f = QFont("Arial", fz, QFont.Weight.Black)
        p.setFont(f)
        if bimetal:
            p.drawText(QRect(0, 0, sz, sz), Qt.AlignmentFlag.AlignCenter, num_only)
        else:
            # Place text slightly lower to avoid star ring
            p.drawText(QRect(0, 6, sz, sz - 6), Qt.AlignmentFlag.AlignCenter, num_only)

    else:  # ── Billetes ──────────────────────────────────────────────────────────
        bill_cfg = {
            "5 €":   ("#6B8E6B", "#3D5C3D", "#A8C8A8", "#8FBC8F"),
            "10 €":  ("#C07840", "#8A4E1A", "#E8A060", "#D4824A"),
            "20 €":  ("#6070C8", "#3848A0", "#8090E0", "#4858B8"),
            "50 €":  ("#D09040", "#A05010", "#E8B050", "#C07020"),
            "100 €": ("#70B878", "#3A7840", "#98D0A0", "#50986A"),
            "200 €": ("#C8A840", "#906010", "#E0C060", "#B09020"),
            "500 €": ("#9870C0", "#604090", "#C0A0E0", "#7858B0"),
        }
        c_main, c_dark, c_light, c_mid = bill_cfg.get(label, ("#607080", "#404858", "#809098", "#506070"))

        # Bill rectangle — landscape, centered vertically
        bw, bh = sz - 4, sz * 9 // 20   # ~76×36 in 80px canvas
        bx = 2
        by = (sz - bh) // 2

        # Shadow
        p.setBrush(QColor(0, 0, 0, 60)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(QRectF(bx + 2, by + 2, bw, bh), 4, 4)

        # Main body
        p.setBrush(QColor(c_main)); p.setPen(QPen(QColor(c_dark), 1.0))
        p.drawRoundedRect(QRectF(bx, by, bw, bh), 4, 4)

        # Left stripe — EU flag area (dark)
        lw = bw * 14 // 76
        clip_path = QPainterPath()
        clip_path.addRoundedRect(QRectF(bx, by, bw, bh), 4, 4)
        p.setClipPath(clip_path)
        p.setBrush(QColor(c_dark)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRect(QRectF(bx, by, lw, bh))

        # EU stars on the left stripe (small)
        stars_cx = bx + lw // 2
        stars_cy = by + bh // 2
        _eu_stars(p, stars_cx, stars_cy, lw * 0.33, n=6, star_r=1.0)

        # Right hologram stripe
        rw = bw * 8 // 76
        p.setBrush(QColor(c_light)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRect(QRectF(bx + bw - rw, by, rw, bh))
        # Hologram shimmer lines
        holo_x0 = bx + bw - rw + 1
        holo_x1 = bx + bw - 2
        p.setPen(QPen(QColor(255, 255, 255, 80), 0.7))
        for yi in range(by + 3, by + bh - 2, 4):
            p.drawLine(holo_x0, yi, holo_x1, yi)

        # Architectural window motif (right-center) — arched rectangles
        win_x = bx + lw + (bw - lw - rw) * 55 // 100
        win_w, win_h = 10, bh - 10
        win_y = by + 5
        p.setBrush(QColor(c_mid)); p.setPen(QPen(QColor(c_dark), 0.7))
        p.drawRoundedRect(QRectF(win_x, win_y, win_w, win_h), 3, 3)
        p.setBrush(QColor(0, 0, 0, 50)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(QRectF(win_x + 2, win_y + 2, win_w - 4, win_h - 4), 2, 2)

        p.setClipping(False)

        # Denomination number — large, left of center
        denom_num = label.replace(" €", "").replace("€", "")
        num_x = bx + lw + 3
        num_w = (bw - lw - rw) * 50 // 100
        p.setPen(QColor("#FFFFFF"))
        fz = 13 if len(denom_num) <= 2 else 11 if len(denom_num) <= 3 else 9
        f = QFont("Arial", fz, QFont.Weight.Black)
        p.setFont(f)
        p.drawText(QRect(int(num_x), by, int(num_w), bh), Qt.AlignmentFlag.AlignCenter, denom_num)

        # "€" symbol small, below number
        p.setPen(QColor(255, 255, 255, 180))
        fe = QFont("Arial", 6, QFont.Weight.Bold)
        p.setFont(fe)
        p.drawText(QRect(int(num_x), by + bh - 9, int(num_w), 9),
                   Qt.AlignmentFlag.AlignCenter, "EURO")

        # Top highlight
        p.setBrush(QColor(255, 255, 255, 30)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(QRectF(bx + 1, by + 1, bw - 2, bh // 3), 3, 3)
