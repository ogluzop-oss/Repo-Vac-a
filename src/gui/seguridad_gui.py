...
class SeguridadWindow(QWidget):
    ...
    def _tab_asignaciones(self):
        ...
        f.addWidget(_btn("Quitar", self._desasignar, danger=True)); f.addStretch()
        f.addWidget(_btn("🔄  Actualizar", self._refrescar_asig, primary=True))
        lay.addLayout(f)
    ...
    def _tab_acl(self):
        ...
        bar.addWidget(_btn("🔄  Actualizar", self._load_acl, primary=True))
        lay.addLayout(bar)
