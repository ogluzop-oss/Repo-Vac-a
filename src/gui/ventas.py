...
def _combo_neon(opciones: list, editable=True):
    cb = _NeonComboBox()
    cb.setEditable(editable)
    cb.addItems(opciones)
    # _NoArrowComboStyle: Fusion base + collapses drop-down indicator to 0 width
    cb.setStyle(_get_no_arrow_style())
    # Neutralise palette so no colour leaks through the button area
    pal = cb.palette()
    _dk = QColor("#161B22")
    for role in (
        ...
        pal.setColor(role, _dk)
    pal.setColor(QPalette.ColorRole.ButtonText,       QColor(TEXTO))
    pal.setColor(QPalette.ColorRole.Highlight,         QColor(CIAN))
    pal.setColor(QPalette.ColorRole.HighlightedText,   QColor(BG))
    cb.setPalette(pal)
    cb.setStyleSheet(_SS_NEON_INPUT + f"""
        QComboBox {{
            background: #161B22;
            color: {TEXTO};
            border: 1px solid {CIAN};
    ...
    cb.view().setStyleSheet(_SS_COMBO_VIEW)
    # Fix white bars: dark background on the popup container frame
    container = cb.view().parentWidget()
    if container:
        container.setStyleSheet(
            f"background: #161B22; border: 2px solid {CIAN}; border-radius: 8px;"
        )
    if editable and cb.lineEdit():
        cb.lineEdit().setStyleSheet(
            f"background: #161B22; color: {TEXTO}; border: none; "
            f"padding: 0px 4px; font-family: 'Segoe UI'; font-size: 11px;"
        )
    return cb
...
# ─────────────────────────────────────────────────────────────────────────────
# Hilo de búsqueda de ventas
# ─────────────────────────────────────────────────────────────────────────────

class _BusquedaHilo(QThread):
    ...
    def run(self):
        try:
            ...
            if f.get("ticket"):
                conds.append("v.id = %s")
                params.append(int(f["ticket"]) if f["ticket"].isdigit() else -1)

            if f.get("articulo"):
                a = f["articulo"]
                conds.append(
                    "EXISTS (SELECT 1 FROM venta_items vi "
                    "WHERE vi.venta_id = v.id AND "
                    "(vi.codigo_articulo = %s OR vi.nombre LIKE %s))"
                )
                params.extend([a, f"%{a}%"])

            if f.get("fecha_desde") and f.get("fecha_hasta"):
                conds.append("DATE(v.fecha) BETWEEN %s AND %s")
                params.extend([f["fecha_desde"], f["fecha_hasta"]])

            if f.get("hora_desde") and f.get("hora_hasta"):
                conds.append("TIME(v.fecha) BETWEEN %s AND %s")
                params.extend([f["hora_desde"], f["hora_hasta"]])

            if f.get("empleado"):
                conds.append("v.empleado LIKE %s")
                params.append(f"%{f['empleado']}%")

            if f.get("caja") and str(f["caja"]).isdigit():
                conds.append("v.numero_caja = %s")
                params.append(int(f["caja"]))

            if f.get("forma_pago") and f["forma_pago"].lower() not in ("todos", "all", ""):
                conds.append("v.forma_pago = %s")
                params.append(f["forma_pago"])

            if f.get("precio_min"):
                conds.append("v.total >= %s")
                params.append(float(f["precio_min"]))

            if f.get("precio_max"):
                conds.append("v.total <= %s")
                params.append(float(f["precio_max"]))

            where = ("WHERE " + " AND ".join(conds)) if conds else ""
            sql = (
                f"SELECT v.id, v.fecha, v.empleado, v.numero_caja, "
                f"v.forma_pago, v.total "
                f"FROM ventas v {where} ORDER BY v.fecha DESC LIMIT 1000"
            )
...
class VentasAnaliticaWindow(QWidget):
    ...
    def _panel_buscar_ventas(self):
        ...
        # Título
        root.addWidget(_lbl(tr("vta.search_title", default="BUSCAR VENTAS"), bold=True, size=15, color=CIAN))
        root.addWidget(_separador())

        # ── Fila 1: Ticket + Artículo ─────────────────────────────────────────
        row1 = QHBoxLayout()
        row1.setSpacing(6)
        lbl_t = _filter_lbl(tr("vta.lbl_ticket", default="Nº Ticket"))
        lbl_t.setFixedWidth(78)
        lbl_t.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        self.inp_ticket = _input_neon(tr("vta.ph_ticket", default="Código de ticket"))
        lbl_a = _filter_lbl(tr("vta.lbl_article", default="Artículo"))
        lbl_a.setFixedWidth(65)
        lbl_a.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        self.inp_articulo = _input_neon(tr("vta.ph_article", default="Código o nombre de artículo"))
        row1.addWidget(lbl_t)
        row1.addWidget(self.inp_ticket)
        row1.addSpacing(14)
        row1.addWidget(lbl_a)
        row1.addWidget(self.inp_articulo)
        root.addLayout(row1)
        ...
        self.hora_desde  = _input_neon(tr("vta.ph_time_from", default="Hora desde (HH:MM)"))
        self.hora_hasta  = _input_neon(tr("vta.ph_time_to", default="Hora hasta (HH:MM)"))
        for lbl_txt, w in (
            (tr("vta.lbl_date_from", default="Fecha desde"), self.fecha_desde),
            (tr("vta.lbl_date_to", default="Fecha hasta"), self.fecha_hasta),
            (tr("vta.lbl_time_from", default="Hora desde"),  self.hora_desde),
            (tr("vta.lbl_time_to", default="Hora hasta"),  self.hora_hasta),
        ):
            ...
            row2.addWidget(lbl)
            row2.addWidget(w)
            if w is not self.hora_hasta:
                row2.addSpacing(10)
        root.addLayout(row2)

        # ── Fila 3: Empleado + Caja + Forma de pago ───────────────────────────
        row3 = QHBoxLayout()
        row3.setSpacing(6)
        # Combos con patrón display/valor: el texto visible se traduce, pero el
        # dato (userData) conserva el valor lógico en español que va a la consulta.
        self.cmb_empleado = _combo_neon([], editable=False)
        self.cmb_empleado.addItem(tr("vta.opt_all_m", default="Todos"), "")

        self.cmb_caja = _combo_neon([], editable=False)
        self.cmb_caja.addItem(tr("vta.opt_all_f", default="Todas"), "")
        for i in range(1, 21):
            self.cmb_caja.addItem(str(i), str(i))

        self.cmb_pago = _combo_neon([], editable=False)
        for _lbl_pago, _val_pago in (
            ...
            (tr("vta.pay_giftcard", default="tarjeta regalo"), "tarjeta regalo"),
        ):
            self.cmb_pago.addItem(_lbl_pago, _val_pago)

        for lbl_txt, w, lbl_w in (
            (tr("vta.lbl_employee", default="Empleado"),      self.cmb_empleado, 72),
            (tr("vta.lbl_register", default="Caja"),          self.cmb_caja,     45),
            (tr("vta.lbl_payment", default="Forma de pago"),  self.cmb_pago,    100),
        ):
            lbl = _filter_lbl(lbl_txt)
        ...
        for lbl_txt, w in ((tr("vta.lbl_price_min", default="Precio mín."), self.inp_precio_min), (tr("vta.lbl_price_max", default="Precio máx."), self.inp_precio_max)):
            ...
            row4.addWidget(lbl)
            row4.addWidget(w)
            if w is not self.inp_precio_max:
                row4.addSpacing(10)
        row4.addStretch()
        root.addLayout(row4)
        ...
        self.btn_buscar  = QPushButton(tr("vta.btn_search", default="BUSCAR"))
        self.btn_limpiar = QPushButton(tr("vta.btn_clear", default="LIMPIAR"))
        self.btn_ticket  = QPushButton(tr("vta.btn_ticket", default="EXPORTAR TICKET"))
        self.btn_regalo  = QPushButton(tr("vta.btn_gift", default="TICKET REGALO"))
        for btn in (self.btn_buscar, self.btn_limpiar, self.btn_ticket, self.btn_regalo):
            btn.setStyleSheet(_SS_BTN_CIAN)
            btn.setFixedHeight(36)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_ticket.setEnabled(False)
        self.btn_regalo.setEnabled(False)
        self.btn_buscar.clicked.connect(self._buscar_ventas)
        self.btn_limpiar.clicked.connect(self._limpiar_filtros)
        self.btn_ticket.clicked.connect(lambda: self._exportar_ticket(con_precios=True))
        self.btn_regalo.clicked.connect(lambda: self._exportar_ticket(con_precios=False))
        for btn in (self.btn_buscar, self.btn_limpiar, self.btn_ticket, self.btn_regalo):
        ...
        self.tbl_ventas.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        self.tbl_ventas.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        ...
        self.tbl_ventas.itemSelectionChanged.connect(self._on_fila_seleccionada)
        _ClipTableTopCorners(self.tbl_ventas)
        root.addWidget(self.tbl_ventas)

        # Estado
        self.lbl_estado_busq = _lbl("", size=10, color="#8B949E")
        root.addWidget(self.lbl_estado_busq)

        # Populate combos from DB on first load
        self._cargar_sugerencias()
        return page

    # ── Lógica Tab 1 ─────────────────────────────────────────────────────────

    def _cargar_sugerencias(self):
        try:
            with obtener_conexion() as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT DISTINCT empleado FROM ventas WHERE empleado IS NOT NULL ORDER BY empleado"
                )
                empleados = [r[0] for r in cur.fetchall()]
            self.cmb_empleado.clear()
            self.cmb_empleado.addItem(tr("vta.opt_all_m", default="Todos"), "")
            for _emp in empleados:
                self.cmb_empleado.addItem(_emp, _emp)
            # Caja: opciones fijas 1-20, no se sobreescriben con datos de BD
        except Exception:
            pass

    def _buscar_ventas(self):
        if self._busqueda_hilo and self._busqueda_hilo.isRunning():
            return
        self.tbl_ventas.setRowCount(0)
        self.lbl_estado_busq.setText(tr("vta.searching", default="Buscando…"))
        self.btn_buscar.setEnabled(False)

        def _fecha_str(qdate: QDate):
            return qdate.toString("yyyy-MM-dd")

        # Valores lógicos vía userData ("" = sin filtro); el texto puede estar traducido.
        pago = (self.cmb_pago.currentData() or "").strip()
        emp  = (self.cmb_empleado.currentData() or "").strip()
        caja = (self.cmb_caja.currentData() or "").strip()

        filtros = {
            "ticket":      self.inp_ticket.text().strip(),
            "articulo":    self.inp_articulo.text().strip(),
            "fecha_desde": _fecha_str(self.fecha_desde.date()),
            "fecha_hasta": _fecha_str(self.fecha_hasta.date()),
            "hora_desde":  self.hora_desde.text().strip() or None,
            "hora_hasta":  self.hora_hasta.text().strip() or None,
            "empleado":    emp  or None,
            "caja":        caja or None,
            "forma_pago":  pago,
            "precio_min":  self.inp_precio_min.text().strip() or None,
            "precio_max":  self.inp_precio_max.text().strip() or None,
        }

        self._busqueda_hilo = _BusquedaHilo(filtros)
        self._busqueda_hilo.resultado.connect(self._poblar_tabla)
        self._busqueda_hilo.error.connect(self._error_busqueda)
        self._busqueda_hilo.start()

    def _poblar_tabla(self, filas):
        self.btn_buscar.setEnabled(True)
        self.tbl_ventas.setRowCount(len(filas))
        for r, (vid, fecha, emp, caja, pago, total) in enumerate(filas):
            t_num = QTableWidgetItem(f"T-{vid:06d}")
            t_num.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            ...
            for c, val in enumerate(
                ...
                if c == 0:
                    ...
                    if c == 5:
                        item.setForeground(QColor(CIAN))
                    self.tbl_ventas.setItem(r, c, item)
        self.lbl_estado_busq.setText(tr("vta.results_found", default="{n} resultado(s) encontrado(s).", n=len(filas)))
        self.btn_ticket.setEnabled(False)
        self.btn_regalo.setEnabled(False)
        self._venta_id_sel = None

    def _error_busqueda(self, msg):
        self.btn_buscar.setEnabled(True)
        self.lbl_estado_busq.setText(tr("vta.error_prefix", default="Error: {msg}", msg=msg))

    def _limpiar_filtros(self):
        self.inp_ticket.clear()
        self.inp_articulo.clear()
        hoy = QDate.currentDate()
        self.fecha_desde.setDate(hoy.addDays(-30))
        self.fecha_hasta.setDate(hoy)
        self.hora_desde.clear()
        self.hora_hasta.clear()
        self.cmb_empleado.setCurrentIndex(0)
        self.cmb_caja.setCurrentIndex(0)
        self.cmb_pago.setCurrentIndex(0)
        self.inp_precio_min.clear()
        self.inp_precio_max.clear()
        self.tbl_ventas.setRowCount(0)
        self.lbl_estado_busq.setText("")
        self.btn_ticket.setEnabled(False)
        self.btn_regalo.setEnabled(False)
        self._venta_id_sel = None

    def _on_fila_seleccionada(self):
        filas = self.tbl_ventas.selectedItems()
        if not filas:
            self.btn_ticket.setEnabled(False)
            self.btn_regalo.setEnabled(False)
            self._venta_id_sel = None
        ...
        item = self.tbl_ventas.item(row, 0)
        if item:
            self._venta_id_sel = item.data(Qt.ItemDataRole.UserRole)
            self.btn_ticket.setEnabled(True)
            self.btn_regalo.setEnabled(True)

    def _exportar_ticket(self, con_precios=True):
        ...
        try:
            ...
            tipo = "ticket" if con_precios else "ticket_regalo"
            carpeta = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "../../documentos/tickets")
            )
            os.makedirs(carpeta, exist_ok=True)
            nombre = f"{tipo}_T{self._venta_id_sel:06d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    def _generar_pdf_ticket(self, venta_id: int, con_precios: bool) -> bytes:
        with obtener_conexion() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT id, fecha, empleado, numero_caja, forma_pago, total FROM ventas WHERE id = %s",
                (venta_id,),
            )
            venta = cur.fetchone()
            cur.execute(
        ...
        # Idioma del documento + traducción IA de nombres de artículo (Nivel 2).
        _lang = i18n.current_language()
        try:
            from src.utils import pdf_fonts
            _FN, _FB = pdf_fonts.fuentes_para(_lang)
        except Exception:
            _FN, _FB = "Helvetica", "Helvetica-Bold"
        try:
            from src.utils import ai_translator
            _nombres_tr = ai_translator.traducir_lote(
                [str(it[1] or it[0] or "") for it in items], _lang, dominio="tpv"
            )
        except Exception:
        ...
        if con_precios:
            c.drawCentredString(W / 2, H - 24 * mm, tr("vta.pdf_sale_ticket", default="TICKET DE VENTA"))
        else:
            c.drawCentredString(W / 2, H - 24 * mm, tr("vta.pdf_gift_ticket", default="TICKET REGALO"))

        y = H - 38 * mm
        c.setFillColorRGB(0.9, 0.9, 0.9)
        c.setFont(_FB, 8)
        c.drawString(5 * mm, y, tr("vta.pdf_ticket_num", default="Ticket: T-{num}", num=f"{venta[0]:06d}"))
        c.drawRightString(W - 5 * mm, y, f"{tr('ticket.register', default='Caja')}: {venta[3] or 1}")
        y -= 5 * mm
        fecha_str = venta[1].strftime("%d/%m/%Y %H:%M") if hasattr(venta[1], "strftime") else str(venta[1])
        c.setFont(_FN, 8)
        c.drawString(5 * mm, y, f"{tr('ticket.date', default='Fecha')}: {fecha_str}")
        y -= 5 * mm
        c.drawString(5 * mm, y, f"{tr('ticket.employee', default='Empleado')}: {venta[2] or '—'}")
        y -= 3 * mm
        c.setStrokeColorRGB(0, 1, 0.78)
        ...
        c.setFont(_FB, 8)
        c.setFillColorRGB(0, 1, 0.78)
        c.drawString(5 * mm, y, tr("vta.pdf_col_item", default="Artículo"))
        c.drawCentredString(W / 2 + 10 * mm, y, tr("vta.pdf_col_qty", default="Cant."))
        if con_precios:
            c.drawRightString(W - 5 * mm, y, tr("vta.pdf_col_total", default="Total"))
        y -= 4 * mm
        ...
        for _idx, (cod, nom, qty, precio, sub) in enumerate(items):
            _nom_tr = _nombres_tr[_idx] if _idx < len(_nombres_tr) else (nom or cod or "")
            nombre_disp = (_nom_tr or nom or cod or tr("vta.pdf_item_fallback", default="Artículo"))[:28]
        ...
        if con_precios:
            ...
            c.setFont(_FN, 8)
            c.setFillColorRGB(0.7, 0.7, 0.7)
            c.drawString(5 * mm, y, tr("vta.pdf_payment", default="Forma de pago: {x}", x=venta[4] or "—"))

        y -= 10 * mm
        c.setFont(_FN, 7)
        c.setFillColorRGB(0.5, 0.5, 0.5)
        c.drawCentredString(W / 2, y, tr("vta.pdf_thanks", default="¡Gracias por su compra!"))
