"""
Adaptador logistica ↔ IOC (Bloque III) — punto oficial por el que logistica resuelve identidad vía IdentityAPI.
Homogéneo: construido sobre el factory único `identidad.adaptador`. Behavior-preserving (empresa_id con
fallback). No accede a Repository/Cache/SQL. Dirección: logistica → IdentityAPI → … → IOC.
"""

from src.services.identidad.adaptador import construir

_A = construir("logistica")
