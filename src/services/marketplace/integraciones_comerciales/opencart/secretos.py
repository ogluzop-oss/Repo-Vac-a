"""
Conector OpenCart · SECRETOS (Fase WEB-19). API Key del REST API vía el `SecretManager` existente. NUNCA en
claro/JSON/variables globales. Runtime: se CIFRA (Fernet) y se retiene por referencia; producción → entorno/
AWS. `credenciales_ref` = NOMBRE base del secreto. La Shop URL NO es secreto (va en la integración).
"""
...
def guardar_runtime(ref, api_key) -> None:
    ...
    _RUNTIME[ref or "OPENCART"] = sm.cifrar(api_key)
...
def api_key(ref=None):
    ...
    return sm.obtener_secreto(f"{ref}_API_KEY") or sm.obtener_secreto("OPENCART_API_KEY")
