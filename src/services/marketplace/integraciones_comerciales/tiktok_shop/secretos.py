"""
Conector TikTok Shop · SECRETOS (Fase WEB-24). Access Token (token de tienda) vía el `SecretManager`
existente. NUNCA en claro/JSON/variables globales/config local. Runtime: se CIFRA (Fernet) y se retiene por
referencia; producción → entorno/AWS. `credenciales_ref` = NOMBRE base del secreto.
"""
...
def guardar_runtime(ref, access_token) -> None:
    ...
    _RUNTIME[ref or "TIKTOK"] = sm.cifrar(access_token)
...
def access_token(ref=None):
    ...
    return sm.obtener_secreto(f"{ref}_ACCESS_TOKEN") or sm.obtener_secreto("TIKTOK_ACCESS_TOKEN")
