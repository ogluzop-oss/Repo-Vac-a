...
def test_bolsa_carrito_y_tramitar(db, fab, monkeypatch):
    ...
    # Buscar en la bolsa (SOLO tarifas locales) → dos tarifas, ordenadas por precio (más barato primero).
    # Columnas: [Proveedor, Precio (€), Precio ref. (€), Desvío %, PVP Sugerido (€), Unidad, Acción].
    w.in_bolsa_art.setText("ARTUI")
    w._buscar_bolsa()
    assert w.tbl_bolsa.rowCount() == 2
    assert w.tbl_bolsa.item(0, 0).text() == "Barato SL"       # proveedor (el más barato arriba)
    assert "6.00" in w.tbl_bolsa.item(0, 1).text()            # precio (con flecha de alerta preventiva)
